...
extern void makeImage(const float *stars, const int *starTree, const int starSize, float *camParam, 
					  const float *magnitude, const int treeLevel, const int M, const int N, const int step, 
					  const cv::Mat csImage, const int G, const float gridStart, const float gridStep, const float2 *hit, 
					  const float2 *viewthing, const float viewAngle, const int GM, const int GN, const int gridlvl,
					  const int2 *offsetTables, const float2 *hashTables, const int2 *hashPosTag, const int2 *tableSizes, 
					  const int otsize, const int htsize);

void cudaPrep(const float *stars, const int *starTree, const int starSize, float *camParam, const float *magnitude, 
			  const int treeLevel, const int M, const int N, const int step, const cv::Mat csImage, const int G, 
			  const float gridStart, const float gridStep, const float2 *hit, const float2 *viewthing, const float viewAngle, 
			  const int GM, const int GN, const int gridlvl, const int2 *offsetTables, const float2 *hashTables, 
